...
description: >
  ...
  Suggest when a plan targets developer-facing products (APIs, CLIs, SDKs,
  libraries, platforms, docs).
allowed-tools:
  - Read
  - Edit
  - Grep
...
**Anti-skip rule:** Never condense, abbreviate, or skip any review pass (1-8) regardless of plan type. Every pass exists for a reason. If a pass genuinely has zero findings, say "No issues found" and move on -- but you must evaluate it.
...
This is NOT a scored pass. It's a checklist of proven patterns from skill DX.
...
After completing the review, recommend next reviews:
